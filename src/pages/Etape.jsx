...
export default function Etape({ etapeId, voyageId, onBack }) {
  ...
  const byCat = CATS.reduce(
    (a, c) => ({ ...a, [c]: spots.filter((s) => s.categorie === c) }),
    {}
  );

  const navUrl = (spot) => {
    const origin = etape?.hotel_adresse
      ? encodeURIComponent(etape.hotel_adresse)
      : 'Current+Location';
    ...
    <div style={s.app}>
      <div style={s.topbar}>
        ...
        <div>
          ...
          <div style={s.sub}>
            {etape?.nuits} nuit{etape?.nuits > 1 ? 's' : ''} · {done}/{total}{' '}
            visités
          </div>
        </div>
        <div style={{ width: 40 }} />
            ...
            <div style={s.miniScroll}>
              {CATS.map((cat) => {
                ...
                return (
                  ...
                  >
                    <div
                      style={{ ...s.miniCatDot, background: CAT_COLOR[cat] }}
                    />
                ...
                {CATS.map((cat) => {
                  ...
                  return (
                    <div key={cat}>
                      <div style={s.catHdr}>
                        <div
                          style={{ ...s.catDot, background: CAT_COLOR[cat] }}
                        />
                        <span style={s.catLbl}>{cat}</span>
                        <span style={s.catCnt}>
                          {catSpots.filter((s) => s.done).length}/
                          {catSpots.length}
                        </span>
                      </div>
                      <div style={s.catHscroll}>
                        {catSpots.map((spot) => (
                          <div
                            ...
                            style={{
                              ...s.spotCard,
                              opacity: spot.done ? 0.5 : 1,
                            }}
                            onClick={() => {
                              if (spot.lat && mapRef.current)
                                mapRef.current.flyTo(spot.lat, spot.lon);
                            }}
                            onMouseEnter={(ev) => {
                          ...
                          >
                            ...
                            {spot.adresse && (
                              <div style={s.spotCardAddr}>{spot.adresse}</div>
                            )}
      ...
      {spotModal && (
        <div
          style={s.overlay}
          onClick={(e) => e.target === e.currentTarget && setSpotModal(null)}
        >
          <div style={s.modal}>
            <div style={s.mhandle} />
            <div
            ...
            >
              ...
              <div
                style={{
                  ...
                  width: 10,
                  height: 10,
                  background: CAT_COLOR[spotModal.categorie] || '#888',
                  marginTop: 8,
                  flexShrink: 0,
                }}
              />
            ...
            <button
              style={s.btnP}
              onClick={() => window.open(navUrl(spotModal))}
            >
              ↗ Y aller{' '}
              {etape?.hotel_adresse
                ? "(depuis l'hôtel)"
            ...
            <button
              style={{
                ...s.btnP,
                marginTop: 8,
                background: spotModal.done
                  ? '#4AAD8A'
                  : 'rgba(255,255,255,0.1)',
                color: spotModal.done ? 'white' : 'rgba(255,255,255,0.7)',
              }}
              onClick={() => {
                toggleSpot(spotModal.id, spotModal.done);
                setSpotModal({ ...spotModal, done: !spotModal.done });
              }}
            >
              {spotModal.done ? '✓ Visité' : 'Marquer comme visité'}
            </button>
            <button style={s.btnG} onClick={() => deleteSpot(spotModal.id)}>
              Supprimer ce spot
            </button>
          </div>
        </div>
      ...
      {modal && (
        ...
        >
          <div style={s.modal}>
                  ...
                  {suggestions.map((r, i) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        setNom(r.name);
                        setSuggestions([]);
                        if (r.categorie) setCat(r.categorie);
                      }}
                    >
                      <div
                        style={{
                      ...
                      >
                        ...
                        {r.categorie && (
                          <span
                            style={{
                              fontSize: 10,
                              color: CAT_COLOR[r.categorie],
                              border: `1px solid ${CAT_COLOR[r.categorie]}`,
                              borderRadius: 10,
                              padding: '2px 8px',
                              flexShrink: 0,
                              marginLeft: 8,
            ...
            <button style={s.btnP} onClick={addSpot}>
              Ajouter
            </button>
            <button
              style={s.btnG}
